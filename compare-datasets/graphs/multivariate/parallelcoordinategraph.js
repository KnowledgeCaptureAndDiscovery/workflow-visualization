(function($) {
	...
	var Parallelcoordinategraph = function(renderTo, seriesNames, dataCopy, datatypes) {
		...
		module.init = function(renderTo, seriesNames, dataCopy, datatypes) {
			$div = renderTo;
			$div.data("dashboard.multivariate.parallelcoordinategraph", module);

			var noData = false;
			dataCopy.forEach(function(singleData) {
				if(noData) return;
		...
		module.initCheckbox = function() {
			renderAllData = $renderCheckbox.checkbox("checked");
	...
	$.fn.dashboard_multivariate_parallelcoordinategraph = function () {
		var args = Array.prototype.slice.call(arguments);
		return this.each(function () {
			if(typeof args[0] == "string") {
				if($.data($(this), "dashboard.multivariate.parallelcoordinategraph") !== undefined) {
					$.data($(this), "dashboard.multivariate.parallelcoordinategraph")[args[0]].apply(null, args.slice(1));
				}
			}
			else {
				(new (Function.prototype.bind.apply(Parallelcoordinategraph, [null, $(this)].concat(args))));
			}
		});
	};
