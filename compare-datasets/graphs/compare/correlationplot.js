(function($) {
	...
	var CorrelationPlot = function(renderTo, namesCopy, dataCopy, typesCopy) {
		...
		module.init = function(renderTo, namesCopy, dataCopy, types) {
			$div = renderTo;
		...
		module.render = function() {
			...
			data.forEach(function(singleData, ix) {
				...
				module.renderIndividual($div.find(".block-" + ix), singleData);
			});
		}

		module.renderIndividual = function(renderTo, data) {
			var noData = false;
			...
			mdsjs.landmarkMDSAsync(mdsjs.convertToMatrix(dists), 2, function(points) {
				...
				module.plot(renderTo, plotData);
			});
		};

		module.plot = function(renderTo, plotData) {
		...
		module.reset = function() {
			...
			$graph.html("Loading...");
			$graph.css("height", "300px").css("line-height", "300px").css("color", "gray");
		};
